...
class UnifiedFeatureEngineer:
    ...
    def get_feature_names(self) -> List[str]:
        ...
        return [
            'user_id', 'product_id',
            # DEBUG MODEL FEATURES (matching train_models_debug.py)
            'order_count', 'reorder_sum', 'reorder_rate'
        ]
    
    def extract_debug_features(self, user_id: str, order_history: List[Dict]) -> pd.DataFrame:
        """
        Extract the exact 3 features that the debug models expect:
        - order_count: Number of times user ordered each product
        - reorder_sum: Number of times user reordered each product  
        - reorder_rate: reorder_sum / order_count
        """
        if not order_history:
            logger.warning(f"No order history provided for user {user_id}")
            return pd.DataFrame()
        ...
        # Calculate debug model features
        # Note: We don't have reorder info in the CSV, so we'll simulate it
        up_features = user_product_history.groupby('product_id').agg({
            'order_id': 'nunique',  # order_count: Number of orders containing this product
        }).rename(columns={'order_id': 'order_count'})

        # Simulate reorder behavior (products ordered multiple times are likely reorders)
        up_features['reorder_sum'] = (up_features['order_count'] - 1).clip(lower=0)
        up_features['reorder_rate'] = up_features['reorder_sum'] / up_features['order_count'].clip(lower=1)

        # Reset index to make product_id a column
        final_features = up_features.reset_index()
        final_features['user_id'] = user_id

        logger.info(f"Generated {len(final_features)} debug model features for user {user_id}")
        return final_features
